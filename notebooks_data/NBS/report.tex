...
Tradiční metody oceňování často spoléhají na netransparentní znalecké posudky nebo na komparativní metody, které mohou být nepřesné kvůli nedostatku dat (viz kapitola~\ref{chap:market_analysis}). Větší dostupnost dat a výpočetního výkonu umožňuje použít statistické metody, které proces oceňování automatizují a zároveň ukazují, jak jednotlivé parametry cenu ovlivňují.

%---------------------------------------------------------------
\section{Cíle práce}
%---------------------------------------------------------------

Hlavním cílem této práce je navrhnout, implementovat a ověřit model pro odhad tržní nabídkové ceny bytů v České republice na základě jejich specifických atributů. Mezi tyto atributy mohou patřit strukturální charakteristiky nemovitosti (např. užitná plocha, počet místností, stav budovy a energetická náročnost), stejně jako lokalizační proměnné zahrnující geografické souřadnice, regionální faktory a dostupnost služeb, jako jsou obchody nebo veřejná doprava. 

Důraz je kladen na identifikaci hlavních faktorů, které ovlivňují nabídkovou cenu bytů, a na vyhodnocení prediktivní přesnosti navrženého modelu. Pro zachování přehlednosti a logické struktury jsou cíle práce rozděleny na teoretickou a praktickou část. Teoretická část je zaměřena na vymezení problematiky, analýzu realitního trhu a představení vybraných metod strojového učení (viz kapitola~\ref{chap:market_analysis}), zatímco praktická část se věnuje sběru a předzpracování dat, návrhu modelů, jejich evaluaci a interpretaci výsledků (viz kapitoly~\ref{sec:data} a~\ref{chap:implementation}).
...
\begin{itemize}
    ...
    \item \textbf{Současné metody oceňování:} Kriticky zhodnotit současné přístupy k oceňování se zaměřením na komparativní metodu a statistické přístupy.
...
Cílovou proměnnou této práce je nabídková cena, tedy cena uvedená v inzerátu. Ta se může lišit od realizované ceny, za kterou byla nemovitost skutečně prodána. Rozdíl mezi oběma hodnotami může vznikat v důsledku vyjednávání, délky inzerce, tržní situace nebo cenové strategie prodávajícího~\cite{CNB2024RealitniTrh,CaseShiller1989}.

\subsection*{Syntéza a shrnutí přístupů}
Z uvedeného přehledu je patrné, že volba metodiky se odvíjí od povahy nemovitosti, účelu ocenění a dostupnosti informací. U bytů je za nejvhodnější a nejpřesnější považován \textbf{porovnávací přístup}, neboť nejlépe odráží skutečné tržní chování a umožňuje nejvěrněji zachytit vliv lokality i specifických vlastností jednotky~\cite{IVSC2022,CNB2024RealitniTrh}.
...
\section{Metody strojového učení pro odhad cen bytů}
\label{chap:ml_theory}

Tato sekce vymezuje teoretický rámec pro návrh, trénování a vyhodnocení prediktivních modelů. Nejprve je formálně definována predikční úloha, poté jsou diskutovány statistické vlastnosti cen nemovitostí (šikmost, heteroskedasticita, prostorová závislost) a jejich důsledky pro volbu transformace cílové proměnné. Následuje specifikace cílové proměnné (včetně logaritmické transformace a Duanova smearing estimatoru) a vstupních proměnných. Dále je představen návrh modelů od jednoduchých benchmarků po flexibilní nelineární metody (XGBoost). Zvláštní pozornost je věnována interpretaci modelů, konformním predikčním intervalům a SHAP hodnotám. Závěr této sekce popisuje způsob validace pomocí křížové validace a výběr chybových metrik.

\section{Definice predikční úlohy}
\label{sec:prediction_task}

Úloha je formulována jako regresní problém v úlohách učení s učitelem. Cílem je odhadnout nabídkovou cenu bytu na základě souboru jeho strukturálních a lokalizačních charakteristik.
...
Rozdělení cen bývá v praxi výrazně pravostranně šikmé: většina nemovitostí se nachází ve středních cenových hladinách, zatímco menší počet velmi drahých jednotek vytváří dlouhý pravý ocas. Z tohoto důvodu se v hedonických aplikacích často pracuje s logaritmem ceny, který snižuje vliv extrémních hodnot a umožňuje chyby interpretovat relativně, nikoli absolutně~\cite{GoodmanThibodeau2003,Miller2022HedonicGAM}.
...
\begin{equation}
    y_i = \text{price total}_i \in \mathbb{R}^+.
\end{equation}
...
Transformovaná cílová proměnná je definována jako
\begin{equation}
    z_i = \ln(y_i).
\end{equation}
Na této škále je v práci trénována sada modelů, konkrétně lineární regrese, ridge regrese, rozhodovací stromy a XGBoost. Výjimkou je pouze globální medián, který pracuje přímo s původní cenou \(y_i\), kde by tento krok byl zbytečný.

Převod predikce z logaritmické škály zpět do korun však není zcela přímočarý: Jensenova nerovnost~\cite{Jensen1906} implikuje, že \(\operatorname{E}[\exp(\varepsilon)] \neq \exp(\operatorname{E}[\varepsilon])\), a prostá exponenciace by proto systematicky podhodnocovala střední hodnotu ceny. Uvažujme obecný log-lineární model \(\ln(y_i) = f(\mathbf{x}_i) + \varepsilon_i\), kde \(f(\cdot)\) je libovolná regresní funkce a \(\varepsilon_i\) označuje reziduální složku. Střední hodnota ceny na původní škále je pak
\begin{equation}
...
Cena za metr čtvereční, tedy \(y_i / \text{plocha}_i\), je v této práci používána pouze jako pomocná analytická veličina pro explorační analýzu, detekci extrémních hodnot a interpretaci výsledků v jednotlivých segmentech trhu. Není však cílovou proměnnou modelů. Výstupy všech predikčních modelů je proto třeba interpretovat jako odhad celkové nabídkové ceny bytu v českých korunách.
...
Modely jsou v experimentu řazeny od nejjednoduššího referenčního přístupu po flexibilnější nelineární metody. Taková posloupnost umožňuje posoudit, zda složitější model skutečně přináší dodatečnou predikční výkonnost oproti jednoduchým benchmarkům.
...
Validace ověřuje, zda se model dokáže zobecnit na dosud neviděná data. Lze tak porovnat jednotlivé varianty modelů a posoudit, do jaké míry bude výsledný model použitelný v reálných podmínkách. Zároveň pomáhá odhalit přeučení a další nedostatky, které by mohly negativně ovlivnit spolehlivost predikce.
...
V kontextu oceňování nemovitostí je často vhodnější metrikou wMAPE, která eliminuje extrémní vliv velmi levných jednotek tím, že chyby váží skutečnou cenou. Jde o poměr součtu absolutních chyb k součtu všech skutečných cen:
\begin{equation}
    wMAPE = \frac{\sum_{i=1}^{n} |y_i - \hat{y}_i|}{\sum_{i=1}^{n} y_i} \times 100\,\%
\end{equation}

\paragraph{Mediánová absolutní procentuální chyba (MedAPE)}
Mediánová absolutní procentuální chyba (často označovaná též jako MedAPE) je robustní alternativou k metrikám založeným na aritmetickém průměru. Místo průměrování se vypočítá absolutní procentuální chyba pro každé pozorování a následně se z těchto hodnot určí medián:

$$\mathrm{MedAPE} = \operatorname{median} \left( \left| \frac{y_i - \hat{y}_i}{y_i} \right| \right) \times 100\,\%$$
...
Výhodou $R^2$ je jeho bezrozměrnost a snadnou srovnatelnost různých typů regresních modelů. V této práci je používán jako doplňková metrika k MAE a wMAPE pro komplexní pohled na kvalitu prokladu dat. Všechny procentuální metriky (MAPE, wMAPE, MedAPE) jsou uváděny vynásobené stem, tedy v~procentech.

%---------------------------------------------------------------
\chapter{Sběr dat a předzpracování}
\label{sec:data}
%---------------------------------------------------------------
Kvalita a relevance vstupních dat přímo ovlivňuje všechny navazující fáze analýzy. Cílem je pořídit konzistentní záznamy, které reprezentativně popisují sledovaný trh.
...
\end{itemize}
Vybraný portál veřejné endpointy poskytuje, takže byl použit API klient (viz~\cite{Sreality}). Sběr probíhal ve třech krocích:
\begin{description}
...
\midrule
Veřejná doprava & 44\,272 & lat/lon \\
Obchody & 3\,286 & lat/lon \\
Centra měst & 14 & lat/lon \\
\bottomrule
\end{tabular}
\end{table}

Datová sada veřejné dopravy obsahuje 44\,272 záznamů kategorizovaných do šesti skupin: autobusová zastávka (40\,413), vlakové nádraží (1\,752), tramvajová zastávka (1\,360), veřejná doprava obecně (613), metro (120) a železniční zastávka (14). Datová sada obchodů čítá 3\,310 záznamů rozdělených na supermarkety (2\,056) a samoobsluhy (1\,253). Pro~14~krajských měst byly manuálně stanoveny souřadnice jejich geometrických center.
...
Již ve fázi sběru dat byly vyloučeny některé odpovědi API, které obsahovaly neplatné odkazy na inzeráty nebo vracely poškozená či neúplná data. Dále byly odstraněny záznamy s chybějícími důležitými atributy, zejména lokalizačními informacemi, protože poloha nemovitosti je jedním z hlavních faktorů při odhadu ceny bytu. Součástí kontroly bylo také odstranění duplicitních inzerátů s identickými atributy, s výjimkou technických identifikátorů a dalších webově specifických polí.
...
\begin{tabular}{>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{5cm}p{5cm}}
\toprule
...
Pro zajištění integrity dat byla implementována podmíněná strategie imputace pro chybějící data nemovitostí (viz obrázek \ref{fig:missings}). Konkrétně atributy \textbf{Has loggia} a \textbf{Has cellar} slouží jako spolehlivé indikátory pro příslušná pole velikostí. Protože API ponechává tyto plošné hodnoty jako \texttt{null} místo nuly, pokud dané vybavení není specifikované, byla explicitně doplněna hodnota 0 pro \textbf{Loggia area} a \textbf{Cellar area} u všech záznamů, kde odpovídající indikátor nabyl hodnoty 0.
\newpage
\section{Explorační analýza dat (EDA)}
Průzkumná analýza dat slouží k počátečnímu porozumění datové sadě, odhalení odlehlých hodnot, chybějících hodnot, tvaru rozdělení proměnných a základních vztahů mezi nimi~\cite{Tukey1977EDA}. Byla provedena pomocí nástrojů Python, zejména knihovny \textit{pandas} pro tabulkové zpracování dat~\cite{McKinney2010Pandas}.

Pro rychlou prvotní kontrolu datasetu byla dále využita automatizovaná profilovací zpráva vytvořená knihovnou \verb|ydata_profiling| \cite{YdataProfiling}. Tento nástroj shrnuje rozložení proměnných, korelace, chybějící hodnoty a základní statistické charakteristiky jednotlivých atributů datasetu.

Další analýza a rozhodnutí o předzpracování byla vedena zjištěními z tohoto reportu. Report zejména pomohl identifikovat potenciální problémy s~kvalitou dat a zvýraznit vztahy mezi jednotlivými atributy.
...
\textbf{Cílová proměnná, Price Total,} je vyjádřena v českých korunách (Kč) a je nabídkovou cenou uvedenou v inzerátu. Hodnoty se pohybují od 578\,314~Kč do 67\,966\,260~Kč. Distribuce je výrazně pravostranně zkosená, což se projeví v tom, že aritmetický průměr (7 844 760~Kč) významně převyšuje medián (6\,550\,000~Kč) (viz obr. \ref{fig:prices_hist}).

Tato šikmost je typická pro realitní trhy, kde malé procento luxusních nemovitostí posouvá průměr výš a vytváří dlouhý pravý ocas. V případě kladné šikmosti obecně platí vztah \uv{průměr > medián > modus}. Přibližně 80 \% nemovitostí je soustředěno v cenovém pásmu 2,75M až 14,00M Kč, které lze považovat za \uv{jádro} trhu.

Distribuce \textbf{Usable Area} (užitná plocha) a \textbf{Total Area} (celková plocha) vykazuje výrazné pravostranné zešikmení (skewness $\approx 3{,}0$), což naznačuje vysokou koncentraci trhu na menších standardních jednotkách. Přibližně 85{,}4 \% inzerátů má užitnou plochu pod 100~$\mathrm{m}^2$ a přibližně 21{,}5 \% odpovídá jednotkám menším než 50~$\mathrm{m}^2$.
...
\textbf{Construction year} (rok výstavby) se pohybuje od 1810 do 2028. Medián je rok 2019, převyšuje průměr 2002, což indikuje, že dataset je zkreslen směrem k moderním výstavbám a projektům vznikajícím v budoucnu. Vyšší hodnoty zde znamenají novější budovy. Novější objekty mají obecně vyšší ceny díky moderním standardům výstavby a lepší energetické náročnosti. Tento vztah je silně podpořen hodnotou Spearmanova pořadového korelačního koeficientu ($\rho = 0{,}5315$, $p = 0{,}0010$), která ukazuje statisticky významnou pozitivní asociaci mezi rokem výstavby a mediánovou cenou za metr čtvereční.

Je však třeba upozornit, že rok výstavby vykazuje nejvyšší podíl chybějících hodnot ze všech numerických atributů (pouze $26{,}3\,\%$ záznamů má tuto hodnotu vyplněnu). Tato neúplnost pravděpodobně není náhodná --- novější developerské projekty rok výstavby inzerují systematicky, zatímco u~starší zástavby bývá často vynechán. Imputace mediánem (2019) proto může u~části starších bytů uměle posouvat signál směrem k~novější výstavbě, což částečně vysvětluje nízkou důležitost tohoto příznaku ve výsledném modelu.
...
Analýza odhalila statisticky významné \uv{snížení ceny u~přízemních jednotek} (Welchův t-test na~logaritmované ceně~za~$\mathrm{m}^2$, $t = -9{,}11$, $p < 0{,}001$). Nemovitosti umístěné v~prvním patře vykazují mediánovou cenu za metr čtvereční přibližně o~13~\% nižší než jednotky ve vyšších patrech. Toto snížení je běžným tržním fenoménem a odráží preference kupujících ohledně soukromí, bezpečnosti a nižšího hluku pro byty ve vyšších patrech. 
Spearmanův korelační koeficient mezi podlažím a mediánovou cenou za metr čtvereční je pozitivní a statisticky významný ($\rho = 0{,}5684$, $p = 0{,}0111$), což poskytuje statistickou evidenci proti nulové hypotéze o~nezávislosti a~podporuje závěr o~kladném vztahu (viz obr.~\ref{fig:floor_price}).

Doplňkové plochy \textbf{Loggia Area} (výměra lodžie) a \textbf{Cellar Area} (výměra sklepu) mají medián 0~$\mathrm{m}^2$, což znamená, že více než 50 \% inzerátů tyto prvky nemá. Průměrné hodnoty (0{,}79~$\mathrm{m}^2$ a 1{,}67~$\mathrm{m}^2$) jsou vyšší kvůli podmnožině nemovitostí s významně většími doplňkovými plochami. Očekává se, že tyto proměnné budou mít mírně pozitivní vliv na \textbf{Price Total}.

Je třeba rozlišit mezi binární indikací přítomnosti těchto vybavení a tím, zda jsou jejich rozměry zaznamenány. Například přibližně 59{,}3 \% inzerátů uvádí přítomnost sklepa, ale pouze 34{,}4 \% má uvedenou konkrétní \textbf{Cellar Area}. To naznačuje, že ačkoli jsou tyto prostory v nabídkách časté, jejich výměra není vždy systematicky uváděna. Důvodem přitom nemusí být pouze nižší informační význam z pohledu prodávajících, ale také skutečnost, že internetové inzertní platformy fungují z velké části jako agregátor nabídek z různých primárních zdrojů, které tyto údaje nemusejí vždy poskytovat.

Samotná přítomnost lodžie v datech nevykazuje kladnou cenovou prémii. Mediánová cena za metr čtvereční je u bytů s lodžií přibližně o 2{,}5~\% nižší než u bytů bez lodžie (Welchův t-test na~logaritmované ceně~za~$\mathrm{m}^2$, $t = -4{,}74$, $p < 0{,}001$). Tento vztah proto nelze interpretovat jako přímý negativní efekt lodžie, ale spíše jako důsledek toho, že lodžie jsou častější v levnějších segmentech, například ve starší panelové zástavbě.

Dataset obsahuje přesné geografické identifikátory ve formě \textbf{Latitude} a \textbf{Longitude}. Pozorování spadají do územních hranic České republiky (Lat: 48{,}60 až 51{,}00; Lon: 12{,}17 až 18{,}77), což potvrzuje geografickou konzistenci. Distribuce však není rovnoměrná -- výrazná prostorová koncentrace je v hlavních městských centrech: přibližně 32{,}2 \% inzerátů připadá na Prahu a největší okresní segment odpovídá přibližně 7{,}1 \% záznamů.

Zahrnutí geografických souřadnic je důležité pro analýzu, neboť lépe zachycuje prostorovou závislost mezi pozorováními. Souřadnice slouží i jako primární klíč pro budoucí doplnění informací o bodech zájmu (POI). Výpočet vzdáleností od veřejné dopravy, zelených ploch nebo obchodních center pomůže modelu zachytit lokální prémiové efekty, které nejsou plně vysvětleny strukturálními atributy.
\begin{table}[ht]
\centering
...
\midrule
\text{price\_total} (K\v{c}) & 11\,258 & 7\,844\,760 & 5\,406\,630 & 578\,314 & 4\,199\,500 & 6\,545\,000 & 9\,590\,000 & 67\,966\,260 \\
\text{latitude} & 11\,258 & 49{,}91 & 0{,}49 & 48{,}60 & 49{,}61 & 50{,}05 & 50{,}13 & 51{,}00 \\
\text{longitude} & 11\,258 & 15{,}09 & 1{,}41 & 12{,}17 & 14{,}36 & 14{,}51 & 16{,}16 & 18{,}77 \\
\text{usable\_area\_m2} ($\text{m}^2$) & 11\,258 & 72{,}35 & 33{,}31 & 11 & 51 & 67 & 84 & 340 \\
\text{total\_area\_m2} ($\text{m}^2$) & 11\,258 & 75{,}78 & 38{,}32 & 3 & 53 & 68 & 88 & 497 \\
\text{loggia\_area\_m2} ($\text{m}^2$) & 10\,578 & 0{,}85 & 2{,}80 & 0 & 0 & 0 & 0 & 103 \\
\text{cellar\_area\_m2} ($\text{m}^2$) & 11\,258 & 1{,}66 & 4{,}00 & 0 & 0 & 0 & 2 & 106 \\
...
\centering
    ...
    \caption{Histogram rozložení cen bytů, měřené v milionech českých korun (Kč), vykazuje výrazné pravostranné vychýlení.}
...
\centering
    ...
    \caption{Houslové grafy ceny za $\mathrm{m}^2$ napříč patry 1–14 (vyloučeny 0{,}1\,\% extrémní hodnoty). Červená čára indikuje pozitivní korelaci mezi úrovní podlaží a mediánovou cenou jednotky.}
...
Pro zkoumání struktury trhu bylo analyzováno frekvenční rozdělení dispozic. Jak ukazuje obr. \ref{fig:disposition}, trh je dominován dispozicemi \emph{2+kk} a \emph{3+kk}, které představují přibližně 24 \% a 21 \% pozorování.

Po agregaci čtyř nejčastějších kategorií -- \emph{2+kk}, \emph{3+kk}, \emph{2+1} a \emph{3+1} -- tyto tvoří přibližně 72{,}5 \% datasetu, což indikuje vysokou reprezentaci malých a středních bytů. Zbylé segmenty, včetně luxusních 5+ pokojových jednotek, tvoří menší část trhu.

Existuje statisticky významná kladná korelace mezi dispozicí a cílovou proměnnou, přičemž Spearmanův koeficient dosáhl hodnoty $\rho = 0{,}378$ ($p < 0{,}0001$, viz obr.~\ref{fig:disposition_price}), avšak rozptyl cen se s rostoucí dispozicí významně zvětšuje kvůli menšímu počtu pozorování ve vyšších kategoriích dispozic a širokému spektru doplňkových prémiových atributů.

Mezi konstrukčními materiály jednoznačně dominuje cihlová zástavba s podílem přibližně 56{,}4 \%, zatímco panelová tvoří menší, ale významný segment (přibližně 20{,}3 \%). 

Volba konstrukčního materiálu je jedním z determinantů nabídkové ceny. Například \emph{Panelová} zástavba bývá cenově dostupnější vzhledem ke své historické vazbě na masivní bytovou výstavbu, zatímco \emph{Cihlová} a moderní \emph{Skeletová} zpravidla vykazují prémii kvalitní výstavby.

\textbf{Energetická náročnost} budov je popsána proměnnými \textbf{energy class} a \textbf{Energy efficiency rating}. Nejčastější třída je \emph{G} (3\,303 nemovitostí), následovaná třídou \emph{B} (3\,098 záznamů). Data ukazují přítomnost tzv. \emph{green premium}  nemovitosti s~nejlepší energetickou třídou (A~a~B) dosahují vyšších mediánových cen za~$\mathrm{m}^2$ než objekty v~třídě~G. Tento rozdíl je statisticky i~věcně významný --- mediánová cena za~$\mathrm{m}^2$ je u~tříd A~a~B přibližně o~30\,\% vyšší než u~třídy~G (Welchův t-test na~logaritmované ceně za~$\mathrm{m}^2$, $t = 41{,}58$, $p < 0{,}001$) --- a~je v~souladu se zahraničními studiemi, které potvrzují pozitivní vliv energetické certifikace na tržní hodnotu nemovitostí~\cite{FuerstMcAllister2011}.
...
Co se týče sklepení a parkovacích možností, přibližně 59{,}1 \% nemovitostí zahrnuje sklep (\textbf{has cellar}) a 13{,}2 \% inzerátů nabízí garáž (\textbf{has garage}). Tato vybavení obvykle zvyšují užitnost a mohou mít pozitivní vliv na cílovou proměnnou (viz tab. \ref{tab:amenity_correlation}).

Zajímavým zjištěním je záporná korelace přitomné lodžie s cenou. To neznamená, že lodžie sama o sobě snižuje cenu; spíše indikuje, že lodžie jsou častější v starších panelových domech, které patří k levnějším segmentům trhu.
...
K analýze vybavenosti nemovitostí byla využita proměnná \textbf{is furnished}. Nejvíce zastoupenou kategorii tvořily nezařízené objekty (kódováno jako False) s celkovým počtem 2 278 pozorování. Těsně následovaly částečně zařízené nemovitosti (kategorie Částečně) čítající 2 254 záznamů, přičemž plně vybavených bytů bylo identifikováno 1 181. Následná cenová analýza ukázala, že plně zařízené bytové jednotky vykazují v~průměru o~8~\% vyšší nabídkovou cenu ve srovnání s~jednotkami nezařízenými. Welchův t-test na~logaritmované ceně zamítá nulovou hypotézu o~shodě středních hodnot na hladině významnosti $\alpha = 0{,}05$ ($t = 4{,}45$, $p < 0{,}001$).

\subsection{Lokalita a občanská vybavenost}

Proměnné identifikující okres (\texttt{district id}) a kraj (\texttt{locality region id}) jsou důležitými vstupy pro uvažované modely. Jak ilustruje grafické znázornění distribuce cen (viz Obr. \ref{fig:price_region}), realitní trh se vyznačuje značnými regionálními disparitami. Dominantním prvkem je zjevná asymetrie mezi Hlavním městem Prahou a ostatními kraji -- mediánové ceny v Praze dosahují přibližně dvojnásobku cen v ostatních krajích. 
...
Pro identifikaci nejrelevantnějších prostorových proměnných byla nejprve provedena explorační analýza pomocí dopředného výběru (Forward Feature Selection) s cílem maximalizace Spearmanova korelačního koeficientu \(\rho\) vůči nabídkové ceně. Souběžně byla testována sada předdefinovaných skupin POI příznaků (doprava, maloobchod, centrum města, všechny kombinace). Z~tohoto srovnání vzešla jako slibná základní varianta se třemi hlavními příznaky (kvalita dopravní obslužnosti, vzdálenost k nejbližšímu obchodnímu domu, počet obchodů v~okruhu 2~km). Následný podrobnější dopředný výběr příznaků (\textit{forward feature selection}) identifikoval optimalizovanou sadu 5 příznaků (viz tab.~\ref{tab:poi_correlation}), která přinesla dodatečné zlepšení predikční přesnosti a byla proto použita pro finální model (viz kap.~\ref{chap:implementation}).

Veškeré vzdálenosti jsou počítány jako ortodromická vzdálenost (vzdušnou čarou) s využitím Haversinova vzorce~\cite{Sinnott1984Haversine}, přičemž poloměr Země je stanoven na \(R = 6371{,}0088\) km.
...
Výpočetní implementace využívá datovou strukturu BallTree z knihovny scikit-learn s metrikou \texttt{haversine}. Souřadnice bodů zájmu jsou nejprve převedeny do radiánů a následně indexovány do stromové struktury. Tento postup umožňuje efektivně dotazovat nejbližší sousedy (\(k\)-NN) i vyhledávání v definovaném poloměru (radius queries).
...
\begin{itemize}
    \item \texttt{transport nearest km} -- vzdálenost k nejbližšímu dopravnímu uzlu bez ohledu na typ.
    \item \texttt{transport count within 500m} -- počet dopravních uzlů v okruhu do 500 metrů.
    \item \texttt{transport count within 1000m} -- počet dopravních uzlů v okruhu do 1 kilometru.
    \item \texttt{metro train nearest km} -- vzdálenost k nejbližší stanici metra nebo železnice, přičemž je aplikován filtr na kategorie \emph{metro}, \emph{train station} a \emph{train stop}.
\end{itemize}
...
Základní proměnné nerozlišovaly mezi jednotlivými formáty maloobchodních prodejen. Rozklad podle atributu \texttt{poi kind} umožnil vytvořit samostatné proměnné pro jednotlivé typy obchodů. Forward selection vybral dva z nich:

\begin{itemize}
    \item \texttt{supermarket nearest km} -- vzdálenost k nejbližšímu supermarketu.
    \item \texttt{convenience nearest km} -- vzdálenost k nejbližší samoobsluze.
\end{itemize}

Tento výběr potvrzuje hypotézu, že typ prodejny ovlivňuje ocenění odlišným způsobem. Zatímco blízkost supermarketu je indikátorem kvalitní občanské vybavenosti, dostupnost samoobsluhy sama o sobě nepřináší cenovou prémii. Proměnná \texttt{supermarket nearest km} vykazuje slabší korelaci (\(\rho = -0.1268\)) než \texttt{convenience nearest km} (\(\rho = -0.2589\)), což odráží odlišné lokalizační vzorce obou typů prodejen.

\paragraph{Průměrná vzdálenost ke třem nejbližším obchodům (KNN)}

Standardní metrika \texttt{nearest km} zachycuje pouze absolutně nejbližší bod. Forward selection identifikoval průměrnou vzdálenost ke třem nejbližším obchodům jako informativnější metriku koncentrace služeb:

\begin{itemize}
    \item \texttt{grocery avg 3 nearest km} -- průměrná vzdálenost ke třem nejbližším obchodům.
\end{itemize}

Tato proměnná vykazuje Spearmanovu korelaci \(\rho = -0.2256\), tedy vyšší než samotná \texttt{grocery nearest km} (\(\rho = -0.1724\)).
...
Pro kvantifikaci vztahu mezi lokalizačními atributy a nabídkovou cenou byl vypočten Spearmanův korelační koeficient. Forward feature selection -- metoda iterativního přidávání příznaků podle největšího marginálního zvýšení absolutní hodnoty korelace, zastavená při poklesu pod stanovenou mez (viz detailní postup v~sekci~\ref{sec:poi_feature_engineering})  identifikovala z~22 kandidátních POI parametrů pět nejvýznamnějších; jejich korelace s~cílovou proměnnou \texttt{price total} shrnuje tabulka~\ref{tab:poi_correlation}.
...
Nejsilnější korelaci vykazuje \texttt{transport quality score} (\(\rho = +0{,}61\)), což potvrzuje, že kvalita dopravní obslužnosti patří mezi hlavní determinanty nabídkové ceny bytu. Záporná korelace u~\texttt{city center travel time min} (\(\rho = -0{,}4294\)) indikuje, že s~rostoucí dobou jízdy do centra nejbližšího krajského města cena klesá. Zbývající tři příznaky zachycují dostupnost maloobchodní sítě. \texttt{convenience nearest km} (\(\rho = -0{,}2589\)) a~\texttt{grocery avg 3 nearest km} (\(\rho = -0{,}2256\)) vykazují slabší negativní vztah, což naznačuje, že blízkost samoobsluh a~vyšší hustota obchodů mírně zvyšují cenu. Nejméně výrazný je vliv \texttt{supermarket nearest km} (\(\rho = -0{,}1268\)), patrně proto, že supermarkety bývají lokalizovány i~v~periferních oblastech s~nižšími cenami, čímž se jejich vliv na cenu tlumí. Celkově výsledky potvrzují, že prostorové příznaky nesou dodatečnou informaci o~ceně nad rámec základních strukturovaných atributů.



\begin{figure}[htbp]
\centering
    \includegraphics[width=0.92\linewidth]{images/categorical_shares.png}
...
\caption{Distribuce dispozic bytů. Sloupcový graf ukazuje převahu moderních otevřených dispozic (kk) nad tradičními (+1), přičemž největší podíl tvoří kategorie 2+kk a 3+kk.}
...
\centering
    ...
    \caption{Houslové grafy ceny za $\mathrm{m}^2$ rozdělené podle dispozice. Distribuce ukazuje mírný pozitivní monotónní trend ($\rho = 0{,}378$) s postupným nárůstem mediánu}
...
\centering
    ...
    \caption{Houslové grafy ceny za $\mathrm{m}^2$ podle stavu budovy. Vizualizace ukazuje prémiovou hodnotu novějších staveb a hustotu trhu, kde \emph{Dobrý} a \emph{Velmi dobrý} stav dominují}
...
\centering
    ...
    \caption{Houslové grafy ceny za $\mathrm{m}^2$ podle kraje}
...
\begin{itemize}
    ...
    \item \textbf{Podmíněná imputace a logická konzistence:} Pokud byt nemá lodžii nebo sklep, jsou odpovídající plochy (\textbf{loggia area m2}, \textbf{cellar area m2}) nastaveny na nulu. Chybějící \textbf{total area m2} je doplněna z \textbf{usable area m2} a zároveň se vytváří binární indikátor \textbf{total area m2 was missing}, který uchovává informaci o původní neúplnosti. V~produkčním prostředí webové aplikace je však uživatel povinen zadat obě plošné hodnoty, takže tento příznak nabývá při ostré inferenci vždy hodnoty~0. Vzhledem k~marginální důležitosti tohoto příznaku v~modelu je dopad na predikci zanedbatelný.
...
Samotná pipeline předzpracování byla navržena odděleně pro jednotlivé skupiny příznaků podle jejich datového typu a role v modelu. Toto rozdělení aplikuje na každý typ proměnných takový způsob zpracování, který nejlépe odpovídá jeho charakteru.

\begin{itemize}
    \item \textbf{Numerické proměnné (Geografie, Plocha a Rok výstavby)}
    \begin{itemize}
        \item \textbf{Postup:} Geografické proměnné \textbf{latitude}, \textbf{longitude} a plošné proměnné (\textbf{usable area m2}, \textbf{total area m2}, atd.) byly zpracovány pomocí imputace mediánem. \textbf{construction year} je přirozeně numerická proměnná a byla standardizována pomocí \textbf{StandardScaler}.
    ...
    \begin{itemize}
        ...
        \item \textbf{Proč:} Nativní podpora kategorií v XGBoost je efektivnější než One-Hot kódování; snižuje paměťovou náročnost a model lépe pracuje s vnitřním rozdělením stromů bez nárůstu dimenzionality.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Postup:} \textbf{district id} byl zpracován pomocí \textbf{SmoothedTargetEncoder} (v rámci pipeline, která je při křížové validaci fitována samostatně v každém foldu pouze na trénovacích datech — encoder nikdy nevidí cílovou proměnnou validačního foldu, čímž se předchází úniku informací).
...
Na základě zjištění explorační analýzy -- pravostranné šikmosti rozdělení cen (viz~obr.~\ref{fig:prices_hist}), nelineárního vztahu mezi užitnou plochou a~cenou (viz~obr.~\ref{fig:price_area}) a~přítomnosti heteroskedasticity (viz~sekce~\ref{subsec:model_features}) -- je cílová proměnná modelována na logaritmické škále. Tato transformace linearizuje vztah cena–plocha, snižuje vliv odlehlých pozorování a~lze chyby interpretovat relativně, nikoli absolutně. Zpětný převod na korunovou škálu je zajištěn pomocí Duanova smearing estimatoru (viz~sekce~\ref{subsec:target_variable}). Stejný princip je aplikován i~na plošné atributy (\texttt{usable area m\textsuperscript{2}}, \texttt{total area m\textsuperscript{2}}, \texttt{loggia area m\textsuperscript{2}}, \texttt{cellar area m\textsuperscript{2}}), které vstupují do~modelu po~logaritmické transformaci.
...
\texttt{construction\_year} & numerická & rok výstavby \\
...
Dále byl vyzkoušen \texttt{Decision tree baseline}, tedy jednoduchý stromový model~\cite{Breiman1984CART} nad stejnou skupinou strukturovaných atributů. Tento model reprezentuje nejjednodušší nelineární přístup v experimentu.
...
Na OOF cross-validaci dosahuje XGBoost přibližně \texttt{MedAPE = 0.0907} a \texttt{R2 = 0.8881}, což je výrazně lepší výsledek než u všech ostatních baseline modelů. Po natrénování na celé trénovací množině a vyhodnocení na testu pak dosahuje přibližně \texttt{MedAPE = 0.0855} a \texttt{R2 = 0.9151}. V experimentu tak vystupuje jako finálně zvolený referenční model pro další část práce.

\section{Finální model}

Z výsledků provedených experimentů byl jako výsledný model zvolen XGBoost. Tento model prokázal nejvyšší schopnost zachytit komplexní nelineární vztahy a interakce mezi strukturními a geografickými parametry nemovitostí. Je vhodné upřesnit, že srovnávací baseline experiment (tab.~\ref{tab:model_comparison_czech_republic}) využívá 300 stromů, zatímco navazující POI experiment pracuje s 800 stromy a odlišnou konfigurací hyperparametrů. Pro dosažení optimální přesnosti byl model obohacen o informace POI, které nebyly zahrnuty u baseline a podroben procesu expertního ladění hyperparametrů (fine-tuning) pro maximální výkonnost.

\subsection{Hyperparametry a strategie ladění}
Nejprve byla základní architektura modelu zvolena porovnáním úvodní sady konfigurací pomocí 5-fold OOF validace. Namísto náhodného zkoušení byl definován prohledávací prostor pokrývající hlavní růstové strategie XGBoostu (hloubkový růst \texttt{depthwise} vs. listový růst \texttt{lossguide}~\cite{ChenGuestrin2016XGBoost}), maximální hloubku stromu v intervalu \(D \in \{6, 8, 10, 12, \text{neomezeno}\}\), a různé úrovně regularizace počtu listů (až do \texttt{max leaves=63}). Z~tohoto porovnání vzešel jako výchozí baseline model XGBoost s \texttt{max depth=8}, \texttt{subsample=0{,}7} a~\texttt{n estimators=800} (viz tab.~\ref{tab:tuning_phases}). Následně bylo provedeno jemné ladění metodou GridSearchCV na modelu obohaceném o~POI příznaky. Optimální hyperparametry:

\begin{itemize}
    \item \textbf{Počet stromů:} 800 s učícím koeficientem 0,025.
    \item \textbf{Strategie růstu:} \texttt{lossguide} s maximálním počtem listů 63 a maximální hloubkou stromu 12 (\texttt{max depth~=~12}). GridSearchCV vyhodnotil hloubku 12 jako optimální ve srovnání s neomezenou hloubkou (\texttt{max depth~=~0}) i hodnotami 8 a 10.
    \item \textbf{Regularizace a vzorkování:} \texttt{subsample~=~0,7}, \texttt{colsample bytree~=~0,8} a L2 regularizace (\texttt{reg lambda~=~1}).
\end{itemize}
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.2\linewidth}ccccc}
\hline
Model & \texttt{max depth} & \texttt{n estimators} & MedAPE (OOF) & \(R^2\) (OOF) \\
\hline
Baseline XGBoost & 12 & 800 & 0{,}0948 & 0{,}8938 \\
XGBoost + top-5 POI & 12 & 800 & 0{,}0868 & 0{,}9006 \\
\hline
\end{tabular}
\end{table}
...
Pro maximalizaci predikčního výkonu byla navržena a implementována sada 5 rozšiřujících POI parametrů pro predikci cen bytů v České republice. Pomocí dopředného výběru příznaků (\textit{forward feature selection}) bylo identifikováno 5 prostorových atributů, které nejvíce přispívají ke~zlepšení přesnosti modelu: skóre kvality dopravního spojení (\texttt{transport quality score}), doba dojezdu do centra kraje (\texttt{city center travel time min}), vzdálenost k~nejbližší samoobsluze (\texttt{convenience nearest km}), průměrná vzdálenost ke~třem nejbližším potravinám (\texttt{grocery avg 3 nearest km}) a vzdálenost k~nejbližšímu supermarketu (\texttt{supermarket nearest km}). Výsledky na nezávislé testovací sadě ukazují následující zjištění (viz tab.~\ref{tab:model_performance}):

\begin{enumerate}
    \item Nejlepšího výkonu dosáhl POI model s pěti top příznaky, který dosahuje \texttt{MedAPE = 0,0816} a potvrzuje tak výrazné a konzistentní zlepšení predikční přesnosti.
    \item Oproti baseline modelu snížil POI model chybu MedAPE z 0,0879 na 0,0816, což je relativní zlepšení o přibližně $7{,}2\,\%$. Tento rozdíl je výrazně větší než u modelu se třemi základními POI příznaky, kde činilo zlepšení pouze $0{,}4\,\%$.
    \item Výsledky prokazují, že prostorové POI příznaky (např. kvalita dopravní obslužnosti či vzdálenost k občanské vybavenosti) nesou dodatečnou informační hodnotu nad rámec základních geografických identifikátorů. Zejména kombinace skóre dopravní kvality a doby dojezdu do centra kraje významně rozšiřuje prediktivní sílu modelu.
\end{enumerate}

V~5-fold OOF cross-validaci dosáhl baseline XGBoost MedAPE~=~0{,}0948, zatímco model obohacený o~POI příznaky dosáhl MedAPE~=~0{,}0868, což je zlepšení o~+8{,}5~\% a R\textsuperscript{2}~=~0{,}9006 (viz tab.~\ref{tab:tuning_phases}).
...
\hline
Model & MedAPE & MAPE & wMAPE & $R^2$ \\
\hline
XGBoost bez POI & 0{,}0879 & 0{,}1340 & 0{,}1200  & 0{,}9125 \\
POI model (5 top příznaků) & 0{,}0816 & 0{,}1259 & 0{,}1166  & 0{,}9147 \\
\hline
\end{tabular}
\end{table}

Skutečnost, že chybová metrika na testovací sadě (MedAPE = $0{,}0879$ pro~baseline) vyšla nepatrně příznivěji než odhad z~křížové validace na trénovací množině (MedAPE = $0{,}0948$), nepředstavuje metodologickou chybu. Zatímco každý fold v~rámci křížové validace trénuje model pouze na~$\frac{4}{5}$ trénovací množiny ($\approx 9\,006$ pozorování), finální model je natrénován na~všech 11\,258 záznamech. Větší trénovací vzorek přirozeně vede k~lepší generalizaci, což vysvětluje mírně příznivější hodnoty na testovací sadě. Model s~pěti POI příznaky dosáhl na testovací sadě MedAPE~=~$0{,}0816$, což je o~$7{,}2\,\%$ lepší výsledek než baseline. Z~akademického hlediska je podstatné, že nedošlo k~degradaci predikčního výkonu, což spolehlivě vyvrací podezření na přeučení (overfitting) modelu.
...
Pro účely globální interpretace a zajištění transparentnosti byl analyzován primárně \textbf{baseline model XGBoost} (bez POI příznaků). Tento postup byl zvolen záměrně, neboť baseline model zachycuje základní cenotvorbu odvozenou výhradně ze strukturovaných vlastností bytu a základních geografických identifikátorů, což poskytuje stabilnější a snáze interpretovatelný referenční rámec pro koncového uživatele.

Ze souhrnného SHAP grafu tohoto modelu (viz obr.~\ref{fig:bee_shap}) vyplývá, že mezi nejdůležitější proměnné patří zejména příznak \textbf{kraj (locality region id)}, dále \textbf{užitná plocha (usable area m\textsuperscript{2})} a \textbf{celková plocha (total area m\textsuperscript{2})}.
...
\subsection{Vliv POI příznaků na predikci}
Pro ověření přínosu prostorových příznaků byla provedena SHAP analýza i~na finálním modelu obohaceném o~top-5 POI příznaky, natrénovaném na~celé trénovací množině a~vyhodnoceném na~testovací sadě (1\,263 pozorování). Na rozdíl od~výše popsaného baseline modelu zde vystupují do~popředí zejména metriky dopravní obslužnosti a~dostupnosti center.

Největší střední absolutní SHAP hodnotu (0{,}1944) si ponechává příznak \textbf{kraj (locality region id)}. Na~druhém místě se však nově umísťuje \textbf{skóre kvality dopravy (transport quality score)} s~hodnotou 0{,}1136, následované užitnou plochou (0{,}1012). Čtvrtou pozici zaujímá \textbf{doba jízdy do~centra kraje (city center travel time min)} s~hodnotou 0{,}0999, čímž se řadí mezi nejsilnější prediktory modelu.

Zbývající POI příznaky  \textbf{průměrná vzdálenost ke~3 nejbližším obchodům (grocery avg 3 nearest km, 0{,}0186)}, \textbf{vzdálenost k~nejbližší samoobsluze (convenience nearest km, 0{,}0097)} a~\textbf{vzdálenost k~nejbližšímu supermarketu (supermarket nearest km)}  vykazují sice nižší SHAP hodnoty, avšak jejich souhrnný efekt doplňuje obraz o~lokální občanské vybavenosti.

Výsledky SHAP analýzy na POI modelu tak potvrzují, že prostorové příznaky nenesou pouze inkrementální zlepšení predikční přesnosti (viz tab.~\ref{tab:model_performance}), ale že některé z~nich -- zejména dostupnost hromadné dopravy a~vzdálenost k centru patří mezi strukturálně nejvýznamnější vstupy modelu.
...
\midrule
XGBoost baseline & 0.0907 & 0.1417 & 0.1291 & 1\,013\,175 & 0.8881 & 11\,258 \\
...
\centering
    ...
    \caption{Dependency plot SHAP hodnot v závislosti na patře (znázorňuje výrazně nižší cenu u bytů v prvním patře)}
...
\begin{figure}[htbp]
\centering
    \includegraphics[width=1\linewidth]{images/shap_poi.png}
    \caption{Beeswarm plot top 15 proměnných s největším příspěvkem na tvorbě ceny u POI XGBoost}
    \label{fig:bee_shap_poi}
\end{figure}
...
Uživatelské rozhraní (UI) je realizováno jako responzivní jednostránková webová aplikace postavená na standardech HTML5, CSS3 a JavaScriptu, bez nutnosti stahovat robustní frontendové rámce, což výrazně snižuje čas do prvního vykreslení. Vstupní formulář je hierarchicky rozdělen do čtyř logických bloků pomocí sémantických značek \texttt{<fieldset>}:
\begin{itemize}
    ...
    \item \textbf{Dispozice a plocha:} Textové a číselné vstupy s omezujícími atributy pro užitnou plochu ($\mathrm{m}^2$), celkovou plochu, typ dispozice a specifické výměry doplňkových prostor (lodžie, sklep).
...
Klientská komponenta provádí asynchronní validaci na události \texttt{input} a \texttt{change}. Implementovaná pravidla kontrolují logickou konzistenci zadaných hodnot, konkrétně:
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Interaktivní waterfall graf vygenerovaný knihovnou Plotly.js na frontendu, vizualizující multiplikativní procentuální příspěvky jednotlivých parametrů k finální odhadované ceně bytu.}
    \label{fig:app_shap}
\end{figure}
%---------------------------------------------------------------
\newpage{}
...
Porovnání modelů prokázalo, že zatímco lineární přístupy poskytují solidní interpretovatelný základ, pokročilé metody strojového učení (XGBoost) lépe zachycují nelineární vztahy a interakce. Prokazatelným přínosem práce je integrace prostorové infrastruktury (POI). Obohacení modelu o metriky, jako je kvalita dopravní obslužnosti či vzdálenost k občanské vybavenosti, snížilo mediánovou chybu odhadu (MedAPE) o přibližně $7{,}2\,\%$ na nezávislé testovací sadě oproti baseline modelu. Ačkoliv se jedná o inkrementální zlepšení, potvrzuje to, že lokální dostupnost služeb hraje v cenotvorbě měřitelnou roli.

Využití SHAP hodnot nad referenčním modelem navíc poskytlo transparentní vhled do logiky odhadů, kde dominantní roli hraje kombinace makro-lokality (okres) a plošných dispozic, doplněná o stav budovy a doplňkové vybavení. 

Praktickým výstupem práce je vytvoření plně kontejnerizovaného, reprodukovatelného systému, který zpřístupňuje komplexní model strojového učení skrze responzivní webové rozhraní s integrovanou asymetrickou kvantifikací nejistoty. V navazujícím výzkumu by bylo vhodné obohatit prostorové indexy o hlubší demografická data či analyzovat nestrukturované vizuální informace z půdorysů bytových jednotek pro přesnější zachycení mikrolokálních anomálií. Pro~přesnější kvantifikaci přínosu nelineárních modelů by bylo rovněž vhodné doplnit benchmark zohledňující lokalitu (např.~mediánovou cenu za~$\mathrm{m}^2$ v~daném okrese vynásobenou užitnou plochou), který by nastavil realističtější dolní mez predikčního výkonu než globální medián.

Dosažené výsledky jsou zatíženy několika omezeními. Především byl sběr dat proveden pouze jednorázově, není proto schopen zachytit vývoj trhu v čase, například změny úrokových sazeb, ekonomických podmínek nebo sezónní výkyvy poptávky. Dalším významným omezením je absence vizuálních informací o stavu a kvalitě interiéru nemovitostí v podobě fotografií. Model tak nedokáže rozlišit mezi čerstvě a nadstandardně zrekonstruovaným bytem a bytem vyžadujícím rozsáhlejší rekonstrukci, pokud oba inzeráty uvádějí stejnou kategorii stavu, například „velmi dobrý“. Tato mezera představuje významný zdroj rozdílu cen, zejména u nemovitostí ve vyšším cenovém segmentu.
%---------------------------------------------------------------
