...
V reálných podmínkách je však tržní cena ovlivněna velkým množstvím vzájemně provázaných faktorů, jejichž manuální vyhodnocování je u rozsáhlých datových souborů neefektivní. Právě komplexita těchto vztahů a potřeba přesnějšího zachycení hedonických atributů vytváří prostor pro využití metod strojového učení, které dokáží identifikovat i nelineární vazby mezi vlastnostmi nemovitosti a její tržní cenou.
...
Ceny nemovitostí mají několik vlastností, které je nutné zohlednit při návrhu predikčního modelu. Především jde o výraznou heterogenitu: byty se liší velikostí, stavem, typem vlastnictví, vybavením i lokalitou, takže nelze očekávat jednoduchý jednotný vztah mezi vstupními proměnnými a cenou. Tato heterogenita je jedním z hlavních důvodů využití hedonických modelů, které chápou cenu jako výsledek kombinace dílčích charakteristik nemovitosti \cite{Lancaster1966,Rosen1974}.
...
V rámci hedonického přístupu je cena nemovitosti chápána jako výsledek kombinace jejích pozorovaných charakteristik a charakteristik okolí, přičemž pozorovaná nabídková cena slouží jako empiricky dostupná aproximace této hodnoty. Vlastnosti rozdělení cen, včetně pravostranné šikmosti a heteroskedasticity, jsou podrobně diskutovány v~sekci~\ref{subsec:price_distribution}. Z~těchto důvodů je v~práci použita logaritmická transformace cílové proměnné.
...
\begin{equation}
    z_i = \ln(y_i).
\end{equation}
Na této škále je v práci trénována většina uvažovaných modelů, konkrétně lineární regrese, ridge regrese, rozhodovací stromy a XGBoost. Výjimku představuje pouze globální medián, který pracuje přímo s původní cenou \(y_i\).
...
\end{equation}
přičemž nerovnost \(\operatorname{E}[\exp(\varepsilon)] \ge \exp(\operatorname{E}[\varepsilon]) = 1\) plyne přímo z~konvexity exponenciální funkce (Jensenova nerovnost \cite{Jensen1906}). Přirozeným bodovým odhadem je proto
\begin{equation}
    \hat{y}_* = \exp(\hat{z}_*) \cdot \widehat{\operatorname{E}}\!\bigl[\exp(\varepsilon)\bigr],
\end{equation}
kde \(\hat{z}_* = \widehat{f}(\mathbf{x}_*)\) je predikce modelu na logaritmické škále. Přirozeným neparametrickým kandidátem pro odhad \(\operatorname{E}[\exp(\varepsilon)]\) je Duanův smearing estimator \cite{Duan1983Smearing}.

Korekční koeficient \(\hat{s}\) je odhadnut z trénovacích reziduí podle vztahu
\begin{equation}
    \hat{s} = \frac{1}{n}\sum_{i=1}^{n}\exp(e_i), \qquad \text{kde } e_i = z_i - \hat{z}_i,
\end{equation}
kde \(\hat{z}_i\) představuje predikci modelu pro \(i\)-té pozorování. Finální bodová predikce ceny nového bytu na původní korunové škále je pak dána vztahem
\begin{equation}
...
Prakticky je však třeba opatrnosti. Koeficienty mohou být ovlivněny škálováním, one-hot kódováním, regularizací i korelacemi mezi vstupními proměnnými. U ridge regrese navíc regularizace záměrně zmenšuje koeficienty, takže jejich absolutní velikost nelze chápat jako čistý ekonomický efekt
daného atributu.
...
\begin{enumerate}
    \item Nejlepšího výkonu dosáhl doladěný POI model, který dosahuje \texttt{MedAPE = 0.0837} a potvrzuje tak mírné, avšak konzistentní zlepšení predikční přesnosti.
    \item Oproti baseline modelu doladěný POI model snížil chybu MedAPE z 0.0855 na 0.0837, což představuje relativní zlepšení o přibližně $2{,}1\,\%$. 
...
\begin{itemize}
    ...
    \item \textbf{Aplikační vrstva (backend)} -- implementována v Pythonu pomocí mikrorámce Flask~3.0 \cite{Flask}, který přijímá požadavky, validuje vstupy, volá doménovou logiku a vrací strukturovanou JSON odpověď. V produkčním režimu je provozována nad WSGI serverem Gunicorn \cite{Gunicorn}.
...
\begin{enumerate}
    \item \textbf{Příjem a serverová validace:} Server přijme strukturovaný JSON payload na endpointu \texttt{/predict}. Modul \texttt{app.py} implementuje striktní defenzivní logiku, která vynucuje přítomnost přesně 23~požadovaných příznaků a provádí explicitní typovou konverzi (přetypování na \texttt{float}/\texttt{int} a sanitaci řetězců), čímž eliminuje riziko neočekávaného selhání doménového jádra či pokusů o injektáž kódu. Pokud libovolný parametr chybí nebo nevyhovuje datovému typu, exekuce se přeruší a API vrátí kód \texttt{400 Bad Request} s výčtem chybějících polí.
    \item \textbf{Obohacení dat (Runtime POI):} Na základě zadaných GPS souřadnic provede server prostorový dotaz (Spatial Join) nad interní, v~paměti lokalizovanou infrastrukturou prostorových indexů (připravenou třídou \texttt{PredictLoader}), což zajišťuje konstantní sub-milisekundovou latenci bez závislosti na externích síťových službách v~reálném čase.
    \item \textbf{Inference a transformace:} Vektor příznaků je předložen předzpracování a Model provede predikci na logaritmické škále, která je následně transformována zpět do původní měnové škály.
...
\subsection{Inicializace a návrhový vzor Singleton}
Z důvodu minimalizace latence je klíčové, aby model a doprovodné datové struktury nebyly načítány dynamicky při každém HTTP požadavku. Inicializace probíhá pomocí návrhového vzoru \textit{Singleton} zapouzdřeného ve třídě \texttt{PredictLoader}, která při startu aplikace načte serializovaný model (\texttt{tuned\_xgb\_pipeline.joblib}), parametry čištění dat, inicializuje prostorové indexy BallTree pro výpočet POI a připraví XGBoost booster pro extrakci SHAP hodnot. Instance je následně sdílena mezi všemi požadavky.
...
Porovnání modelů prokázalo, že zatímco lineární přístupy poskytují solidní interpretovatelný základ, pokročilé metody strojového učení (XGBoost) lépe zachycují nelineární vztahy a interakce. Prokazatelným přínosem práce je integrace prostorové infrastruktury (POI). Obohacení modelu o metriky jako je kvalita dopravní obslužnosti či vzdálenost k občanské vybavenosti snížilo mediánovou chybu odhadu (MedAPE) o přibližně $2{,}1\,\%$ oproti baseline modelu. Ačkoliv se jedná o inkrementální zlepšení, potvrzuje to, že lokální dostupnost služeb hraje v cenotvorbě měřitelnou roli.
